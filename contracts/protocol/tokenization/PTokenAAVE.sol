...
/**
 * @title Prestare ERC20 PToken wrap AAVE aToken
 * @dev Implementation of the interest bearing aToken for the Prestare protocol
 */
  ...
  IPToken
{
  ...
  function burn(
    ...
    IERC20(_underlyingAtoken).safeTransfer(receiverOfUnderlying, amount);
   ...
   * @param index The new liquidity index of the reserve
   */
  function mintToTreasury(uint256 amount, uint256 index) external override onlyCounter {
    if (amount == 0) {
      return;
    }
   ...
   * @return The underlying asset balance of the user E.g. WETH for paWETH
   **/
  ...
  /**
   * @dev Returns the address of the underlying atoken asset of this pToken(E.g. aWETH for paWETH)
   */
  function UNDERLYING_ATOKEN_ADDRESS() public view returns (address) {
    return _underlyingAtoken;
  }
   ...
   **/
  function handleRepayment(address user, uint256 amount) external override onlyCounter {}
  ...
  /**
   * @dev Transfers the pTokens between two users. Validates the transfer
